...
async function seedSpecialAccounts() {
  ...
  for (const s of specials) {
    ...
    if (exists.rows.length === 0) {
      ...
      // Force-reset special accounts to correct values on every deploy
      await query(
        `UPDATE accounts SET role=$1, is_supreme=$2, coins=500, volts=0,
         skins='["default"]', active_skin='default', is_banned=false,
         battlepass='{"level":1,"xp":0,"season":1}'
         WHERE username=$3`,
        [s.role, s.is_supreme, s.username]
      );
    }
  }
